set @startDate := '2018-06-21 00:00:00';
...
from
	(select 	inscricao.patient_id,
	    ...
	    if(activo_pre_tarv.program_id is null ,null ,'obitou') estado_pre_tarv,
	    ...
	    if(activo_tarv.program_id is null ,null ,'activo no programa') estado_tarv
   -- if(abandono.program_id is null,null,'SIM') abandono_or_transferido
		...
		/*	left join
			(
				...
				from 	patient p
						...
						inner join patient_state ps on pg.patient_program_id=ps.patient_program_id
				where 	pg.voided=0 and ps.voided=0 and p.voided=0 and
						pg.program_id in (1) and ps.state in (2,3,5,7,9,10) and
						ps.start_date <= @endDate  and location_id=@location
				group by pg.patient_id
			...
			left join
			(
				select 	pg.patient_id,max(ps.start_date) data_activo_pre_tarv,pg.program_id,ps.state
				from 	patient p
						inner join patient_program pg on p.patient_id=pg.patient_id
						inner join patient_state ps on pg.patient_program_id=ps.patient_program_id
				where 	pg.voided=0 and ps.voided=0 and p.voided=0 and
						pg.program_id =1 and ps.state in (5) and
						ps.start_date <= @endDate and location_id=@location
				group by pg.patient_id
			...
			left join
			(
				select 	pg.patient_id,max(ps.start_date) data_activo_tarv,pg.program_id
				from 	patient p
						inner join patient_program pg on p.patient_id=pg.patient_id
						inner join patient_state ps on pg.patient_program_id=ps.patient_program_id
				where 	pg.voided=0 and ps.voided=0 and p.voided=0 and
	...
	)inscritos   where   inscritos.estado_pre_tarv = 'obitou'  and inscritos.estado_tarv='activo no programa'
group by patient_id