...
select 	
	   ...
	   -- 	terminoprofilaxiainh.termino_profilaxia_inh,
        ...
        concat(14 * round(round(datediff(@endDate,pe.birthdate)/365) / 14), '-', 14 * round(round(datediff(@endDate,pe.birthdate)/365) / 14) + 14) as `faixa etaria`,
       count(*) as `groups of users`
...
select 	e.patient_id,  min(o.obs_datetime) as inicia_profilaxia_inh, e.encounter_datetime as data_consulta_inicio_inh
    ...
    group by 1
order by round(datediff(@endDate,pe.birthdate)/365);